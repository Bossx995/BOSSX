...
const adminCommands = new Set([
  "anticall","antibot","antidelete","anticlean","antiadmin","antisticker","antimention","antitag","anti-link","antilink",
...
async function handleMessage(sock, msg) {
  ...
  // ANTI-STICKER: delete stickers immediately, only in groups where it was enabled.
  if (!isSelfMessage && isGroup(jid) && isAntiStickerEnabledForGroup(jid)) {
    ...
    if (unwrapped?.stickerMessage) {
      try {
        ...
        logger.error({ err: e, jid, sender, senderPn }, "anti-sticker delete failed");
      }
      return;
    }
  }
  ...
  if (!isSelfMessage && isAntiLinkEnabledForGroup(jid) && containsLinkInMessage(msg)) {
    try {
      ...
      if (!senderIsAdmin) {
        ...
        await sock.sendMessage(jid, {
          text: kicked
            ? `🚫 ANTI-LINK\n@${baseNumber(sender)}-এর link message delete করা হয়েছে এবং তাকে group থেকে remove করা হয়েছে।`
            : deleted
...
https://chat.whatsapp.com/${code}`);
      ...
      if (!isGroup(jid)) {
        ...
        if (!value) {
          ...
          await sendBotReply(sock, jid, "👋 এই group-এর welcome message default-এ reset হয়েছে ✅");
        } else {
          if (value.length > 4000) {
            ...
            await sendBotReply(sock, jid, `👋 এই group-এর welcome message saved ✅\n\n${value}`);
          }
        }
      }
      ...
      if (!isGroup(jid)) {
        ...
        if (!value) {
          ...
          await sendBotReply(sock, jid, "🚪 এই group-এর goodbye message default-এ reset হয়েছে ✅");
        } else {
          if (value.length > 4000) {
            ...
            await sendBotReply(sock, jid, `🚪 এই group-এর goodbye message saved ✅\n\n${value}`);
          }
        }
      }
...
async function startBot() {
  ...
  sock.ev.on("group-participants.update", async update => {
    ...
    try {
      ...
      if (action === "add" && Object.prototype.hasOwnProperty.call(settings.welcomeGroups, id)) {
        const metadata = await getGroupMetadataSafe(sock, id);
        const groupName = metadata?.subject || "this group";
        await sendWelcomeMessage(sock, id, participants, groupName).catch(err =>
          logger.error({ err, id }, "welcome message failed")
        );
      } else if (action === "remove" && Object.prototype.hasOwnProperty.call(settings.goodbyeGroups, id)) {
        const metadata = await getGroupMetadataSafe(sock, id);
        const groupName = metadata?.subject || "this group";
        await sendGoodbyeMessage(sock, id, participants, groupName).catch(err =>
          logger.error({ err, id }, "goodbye message failed")
        );
      }
